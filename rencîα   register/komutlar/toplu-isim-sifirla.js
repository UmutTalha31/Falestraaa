...
discowsend(`${ok} **Herkesin İsim Geçmişi Başarıyla Sıfırlandı.** ${tik}`)

db.all().filter(data => data.ID.startsWith("İsimler&")).forEach(data => {
            db.delete(data.ID)
})

}
exports.conf = {
    aliases: ['toplu-isimler-sifirla', 'toplu-isimler'],
  };
  
  exports.help = {
    name: 'Toplu İsimler Sifirla Komutu',
  };